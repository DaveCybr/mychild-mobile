...
class AppRouter {
  ...
  static String? _redirectLogic(BuildContext context, GoRouterState state) {
    ...
    final isAuthenticated = localStorage.getString('user_token')?.isNotEmpty ?? false;
    
    // Check family connection status
    final hasFamilyConnection = localStorage.getString('family_id')?.isNotEmpty ?? false;
    
    // Check permission setup status
    final hasPermissionSetup = localStorage.getBool('permission_setup_completed') ?? false;
